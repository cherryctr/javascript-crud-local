...
function Read(){
 	...
 	if (names2 != null) {
 		for (var i =0;i<names2.length;i++) {
 			userTR.innerHTML +=`
 				<div class="bg-dark border-success text-white card mb-2">
 					<div class="card-body">
 						<p><i class="fa fa-user-circle-o"></i> User : ${names2[i]} </p>
...
function Update(i3){
	...
	for(var i=0;i<names4.length;i++){
		if (i == i3) {
			userTR.innerHTML+=`
				<div class="bg-dark border-danger text-white card mb-2">
 					<div class="card-body">
 						<p><i class="fa fa-user-circle-o"></i> User : </p>
 						<input class="mb-2 form-control" id="newName" placeholder="${names4[i]}">
 						<button class="col-5 text-white btn btn-success" onclick="Update2(${i})">Update</button>
